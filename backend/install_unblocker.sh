...
docker pull wzxjohn/appleauto_backend
docker run -d --name=appleauto --log-opt max-size=1m --log-opt max-file=2 --restart=always --network=host -e API_URL=$api_url -e API_KEY=$api_key -e SYNC_TIME=$sync_time -e AUTO_UPDATE=$enable_auto_update -e LANG=$language -v /var/run/docker.sock:/var/run/docker.sock wzxjohn/appleauto_backend
if [ $language = "1" ]; then
  echo "安装完成，容器已启动"
